...
namespace {
	...
	void promptRestart()
	{
		auto text = std::format(L"{} {}\n\n{}", L"update.found", newVer, L"update.tip");
		auto btnId = MessageBox(nullptr, text.data(), L"about.sysTip", MB_OKCANCEL | MB_ICONINFORMATION | MB_TOPMOST | MB_SETFOREGROUND);
	...
	{
		if (!isIdle()) return;
		auto text = std::format(L"{} {}\n\n{}", L"update.found", verStr, L"update.noPermission");
		auto btnId = MessageBox(nullptr, text.data(), L"about.sysTip", MB_OKCANCEL | MB_ICONINFORMATION | MB_TOPMOST | MB_SETFOREGROUND);
